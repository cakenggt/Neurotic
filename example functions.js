///////////Utility functions/////////
function parseDataset(raw_str){
  var result = [];
  var entries = raw_str.split('\n');
  var enumAttributeList = [];
  for (var x = 0; x < entries.length; x++){
    ...
    for (var y = 0; y < attributes.length; y++){
      ...
      var parsedAttribute = parseFloat(attribute);
      if (isNaN(parsedAttribute)){
        //if not a float value
        if (attribute == '?'){
          //replace with 0
          parsedAttribute = 0;
        }
        else{
          if (enumAttributeList[y] == undefined){
            enumAttributeList[y] = [];
          }
          var enumListForIndex = enumAttributeList[y];
          //see if it is in the enum list for this index
          if (enumListForIndex.indexOf(attribute) != -1){
            //enum has already been registered
            parsedAttribute = enumListForIndex.indexOf(attribute);
          }
          else{
            //register enum and use returned length - 1
            parsedAttribute = enumListForIndex.push(attribute)-1;
          }
        }
      }
      entryList[y] = parsedAttribute;
    }
    result[x] = entryList;
  }
  return result;
...
var additionalIris = [[[5.1,3.5,1.4,0.2],[0]],
[[4.9,3.0,1.4,0.2],[0]],
[[4.7,3.2,1.3,0.2],[0]],
[[4.6,3.1,1.5,0.2],[0]],
[[5.0,3.6,1.4,0.2],[0]],
